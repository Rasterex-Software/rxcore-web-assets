...
var foxitViewer = function foxitViewer(zsdivid, divnum, libpath) {
    ...
    this.getBlobfromURL_old = function (url, filename){
...
this.getThumbnail = function (pagenum) {
    ...
    this.pdfUploadonly = function (bcustom, endpoint) {
        if (foxview.pdfViewer) {
            const pdfDoc = foxview.pdfViewer.getCurrentPDFDoc();
            ...
            pdfDoc.extractPages([[0, pdfDoc.getPageCount() - 1]]).then((doc) => {
               ...
               if(bcustom){
                RxCore.uploadOnlyCustomPDF({filename : uploadname, binfile : blob}, bcustom, endpoint);
               }else{
                RxCore.uploadOnlyCustomPDF({filename : uploadname, binfile : blob});
               } 
    ...
    this.getPDFpagecoordinates = function(pmousepos, npagenum, callback){
        ...
        if(foxview.curpagerender != null){
            pgscale = foxview.curpagerender.getScale();
            pagerotation = foxview.curpagerender.page.getRotationAngle();
        }
        ...
        if (foxview.pdfViewer){
            ...
            foxview.pdfViewer.getCurrentPDFDoc().getPageByIndex(npagenum).then(function (page) {
                ...
                var orgpointarray = [pmousepos.x, pmousepos.y];
                var pdfpoint = page.reverseDevicePoint(orgpointarray, scale, pagerotation);
    ...
    this.onRenderSuccess = function (pdfViewer, ViewerEvents) {
        ...
        if (pdfViewer) {
            pdfViewer.eventEmitter.on(ViewerEvents.renderPageSuccess, function (pageRender) {
                ...
                var pdfrect = pageRender.getHandlerDOM().getBoundingClientRect();
